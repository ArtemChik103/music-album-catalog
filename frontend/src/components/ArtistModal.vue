...
import { PhX, PhUsers } from '@phosphor-icons/vue'
...
watch(
  ...
  (open) => {
    if (open) {
      name.value = props.artist ? props.artist.name : ''
      error.value = ''
    }
  }
)
...
const handleSubmit = () => {
  ...
  if (!trimmed) {
    error.value = 'Имя исполнителя обязательно для заполнения'
    return
  }
  error.value = ''
  emit('save', { id: props.artist?.id, name: trimmed })
}
      ...
      <div class="w-full max-w-md rounded-2xl bg-zinc-900 border border-zinc-800 p-6 shadow-2xl relative">
        ...
        <div class="flex items-center gap-3 mb-5">
          ...
          <h3 class="text-base font-semibold text-zinc-100">
            {{ artist ? 'Редактировать исполнителя' : 'Новый исполнитель' }}
          </h3>
        </div>
        ...
        <form @submit.prevent="handleSubmit" class="space-y-4">
          <div>
            <label class="block text-xs font-medium text-zinc-300 mb-1.5">
              Имя артиста или название группы
            </label>
            <input
              v-model="name"
              type="text"
          ...
          <div class="pt-2 flex items-center justify-end gap-3">
            ...
            >
              {{ isLoading ? 'Сохранение...' : (artist ? 'Сохранить' : 'Добавить') }}
            </button>
          </div>
        </form>
      </div>
